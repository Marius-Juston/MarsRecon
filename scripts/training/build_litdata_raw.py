...
Emits a raw-format LitData stream (elevation / bounds / CRS / orthoimage tiles,
no sun-vector estimation, no residual normalization). For the processed
training format (image/dtm/confidence/sun_vector/...), use `build_litdata.py`.

Usage:
    PYTHONPATH=src uv run python scripts/training/build_litdata_raw.py --config configs/train_hirise.yaml --workers 96
"""
import argparse
import glob
import json
