...
#
#         # Learnable blend between Lambertian (1.0) and Lommel-Seeliger (0.0)
#         # Initialized to 0.0 (which becomes 0.5 after the sigmoid in forward pass)
#         self.lunar_lambert_weight = nn.Parameter(torch.tensor(0.0), requires_grad=False)
#
...
#
#         # ---------------------------------------------------------
#         # 3. Learnable Lunar-Lambert Render
#         # ---------------------------------------------------------
#         # Cosine of incidence angle
#         cos_i = torch.sum(normals * l_dir, dim=1, keepdim=True)
#         # Clamp shadows to strictly >= 0
#         cos_i_clamped = torch.clamp(cos_i, min=0.0)
#
#         # Cosine of emission angle (Z-normal for nadir view)
#         cos_e = normals[:, 2:3, :, :]
#
#         # Calculate pure photometric components
#         lambert_comp = cos_i_clamped
#         ls_comp = cos_i_clamped / (cos_i_clamped + cos_e + 1e-6)
#
#         # Blend using learnable parameter (constrained 0 to 1 via sigmoid)
#         L = torch.sigmoid(self.lunar_lambert_weight)
#         render_blend = (L * lambert_comp) + ((1.0 - L) * ls_comp)
#
#         # Apply Scene Intensity and Ambient
#         render = (render_blend * intensity) + ambient
#
#         # Clamp to mimic actual camera sensor bounds (-1 to 1 based on dataloader)
#         render = torch.clamp(render, min=-1.0, max=-1.0 if False else 1.0)  # Ensure upper bound is 1.0
#         # ---------------------------------------------------------
#
#         # 4. Masked Pearson Correlation with Safety Checks
#         valid_mask = mask.bool()
#
#         # Count valid pixels per image. Shape: (B,)
#         valid_counts = valid_mask.view(B, -1).sum(dim=1)
#         total_pixels = H * W
#
#         # Pass 1: Require at least 25% of the patch to be valid data
#         valid_batch_items = valid_counts > (total_pixels * 0.25)
#
#         if not valid_batch_items.any():
#             return 0.0 * self.lunar_lambert_weight
#
#         # Zero out invalid pixels so they don't affect sums
#         r_masked = render * valid_mask
#         o_masked = ortho_gray * valid_mask
#
#         # Compute means per-image. Shape needed for broadcasting: (B, 1, 1, 1)
#         # We add a tiny epsilon to valid_counts to safely divide, even for invalid items
#         counts_broadcast = valid_counts.view(B, 1, 1, 1) + 1e-8
#         r_mean = r_masked.view(B, -1).sum(dim=1).view(B, 1, 1, 1) / counts_broadcast
#         o_mean = o_masked.view(B, -1).sum(dim=1).view(B, 1, 1, 1) / counts_broadcast
#
...
#
#         # Pass 2: Check for zero-variance (flat renders or pitch-black shadows)
#         min_variance = 1e-5
#         has_variance = (var_r > min_variance) & (var_o > min_variance)
#
#         # Final valid mask isolates items that have enough data AND have structural variance
#         final_valid_items = valid_batch_items & has_variance
#
#         if not final_valid_items.any():
#             return 0.0 * self.lunar_lambert_weight
#
...
#
#         # Average the loss ONLY over the healthy, valid batch items
#         loss = 1.0 - correlation[final_valid_items].mean()
#
#         return loss
