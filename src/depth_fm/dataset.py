"""
Mars DTM Dataset for DepthFM training.

Supports two modes:
1. Raw mode: loads image + DTM TIFFs, applies augmentation, returns pixel tensors
2. Latent mode: loads pre-computed VAE latents from disk (much faster training)

Stereo augmentation: if use_stereo_augmentation=True, both L and R images are
treated as separate training samples paired with the same GT DTM.
"""
